...
namespace AudioConvert.Services
{
    ...
    {
        private const string ProductStoreId = "9N9MRSLLPJ1G";
        ...
        public async Task<ConversionQuotaResult> SignInAsync()
        {
            ...
            ConversionQuotaResult signInResult = await AuthenticateMicrosoftAccountAsync();
            ...
            return await GetBalanceAsync("Microsoft 账号已登录，次数余额已更新。");
        }
        ...
        private async Task<ConversionQuotaResult> AuthenticateMicrosoftAccountAsync()
        {
            try
            {
                var authenticator = new OnlineIdAuthenticator();
                InitializeOnlineIdAuthenticatorWindow(authenticator);

                var ticketRequests = new[]
                {
                    new OnlineIdServiceTicketRequest("wl.basic", "DELEGATION")
                };

                UserIdentity identity = await authenticator.AuthenticateUserAsync(
                    ticketRequests,
                    CredentialPromptType.PromptIfNeeded);

                if (identity is null)
                {
                    return ConversionQuotaResult.Canceled("已取消 Microsoft 账号登录。");
                }

                return ConversionQuotaResult.Success(null, "Microsoft 账号已登录。");
            }
            catch (Exception exception)
            {
                return ConversionQuotaResult.Failure("Microsoft 账号登录失败：" + FormatExtendedError(exception));
            }
        }
        ...
        private bool InitializeOnlineIdAuthenticatorWindow(OnlineIdAuthenticator authenticator)
        {
            IntPtr windowHandle = _windowHandleProvider();
            if (windowHandle == IntPtr.Zero)
            {
            ...
            return InitializeObjectWithWindow(authenticator, windowHandle);
        }
